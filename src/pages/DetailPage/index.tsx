...
export default function DetailPage() {
    ...
    // 출판일
    const [productPublicationDate, setProductPublicationDate] = useState<string>('');
    // 가격
    ...
    useEffect(() => {
        axios.get(`http://localhost:4080/api/book/${productSeq}`)
            .then((response) => {
                const data = response.data;
    ...
    return (
        <>
            <Box display={'flex'} justifyContent={'center'}>
                ...
                <Box border={1} paddingTop={6}>
                    <Box borderBottom={1} padding={1}>
                        <Typography fontSize={20} padding={1}>{productTitle}</Typography>
                        <Typography padding={1}>{productWriter}/{productPublisher}</Typography>
                        <Typography padding={1}>{productPublicationDate}</Typography>
                    </Box>
                    <Box >
                        <Box >
                            <Box display={'flex'} justifyContent={'space-between'} borderBottom={1} padding={1}>
                                <Typography padding={1}>금액</Typography>
                                <Typography padding={1}>{productPrice}원</Typography>
                            </Box>
                            <Box display={'flex'} justifyContent={'space-between'} borderBottom={1} padding={1}>
                                <Typography padding={1}>할인금액</Typography>
                                <Typography padding={1}>{productSalesPrice}원</Typography>
                    ...
                    <Box display={'flex'} justifyContent={'space-between'} pt={5}>
                        <Box border={1} ml={3}>
                            <Link to={"/orderPayment"}>
                                <Button onClick={() => buyingHandler()}>구매하기</Button>
                            </Link>
                        </Box>
                        <Box border={1}>
                    ...
                    <TabContext value={Value}>
                        <Box display={'flex'} justifyContent={'center'} sx={{ borderBottom: 1, borderColor: 'divider' }}>
                            <TabList onChange={(handleChange) => {}} aria-label="lab API tabs example" >
                                <Tab style={{ fontSize: '25px' }} label="책 설명" value="1" />
                                <Tab style={{ fontSize: '25px' }} label="상품 리뷰" value="2" />
                                <Tab style={{ fontSize: '25px' }} label="판매 정보" value="3" />
                            </TabList>
                        </Box>
                        <TabPanel value="1">
                            <Box>
                        ...
                        <TabPanel value="3">
                            <Box>
                                ...
                                <Typography fontSize={18}>
                                    배송방법
                                    택배 / CJ 대한통운 네이버페이 구매 후 반품/교환 시 우체국 택배로 자동 수거되지 않으며, 웅진책방 전용 택배사인 CJ대한통운으로 수거처리 됩니다.
                                </Typography>
                            </Box>
                            <Box>
                                <Typography borderBottom={1} fontSize={18}>
                                    배송비
                                    기본 배송비 3,000원 (제주/도서산간 지역의 경우 추가비용발생) 30,000원 이상 무료 (단, 네이버페이와 같은 간편결제시 제주/도서산간 지역의 경우 추가비용이 발생합니다.)
                                </Typography>
                            </Box>
                            <Box>
                                <Typography fontSize={25}>
                                ...
                                <Typography borderBottom={1} fontSize={18}>
                                    반품/교환 방법
                                    마이페이지 반품/교환 신청 및 조회, 고객센터(1670-3780)로 유선 연락
                                    반품/교환 가능 기간
                                    출고변심반품의 경우 수령 후 7일 이내 재화등의 내용이 표시·광고의 내용과 다르거나 계약내용과 다르게 이행된 경우에는 그 사실을 안 날 또는 알 수 있었던 날부터 30일 이내에 청약철회가 가능 (단, 수령일로 부터 최대 3개월)
                                </Typography>
                            </Box>
                            <Box>
                                <Typography fontSize={25}>
                                ...
                                <Typography borderBottom={1} fontSize={18}>
                                    고객의 단순변심 및 구매 착오로 인한 상품 반송비용은 고객 부담
                                </Typography>
                            </Box>
                            <Box>
                                <Typography fontSize={25}>
                                ...
                                <Typography borderBottom={1} fontSize={18}>
                                    소비자에게 책임이 있는 사유로 재화등이 멸실되거나 훼손된 경우. 다만, 재화등의 내용을 확인하기 위하여 포장 등을 훼손한 경우는 제외
